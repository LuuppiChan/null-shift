...
async def get_dynamic_context(data: LocalData) -> str | None:
    """
    Get the dynamic context for the current message.
    Format:
    <context>
    stuff...
    </context>
    """
    return await dynamic_context.get(data)


class DynamicContext:
    def __init__(self) -> None:
        config = manager.get_config()
        self.path = Path(config.dynamic_context.path)
        self.prompt_cache: dict[Path, CachedFile] = {}
        manager.config_updated.connect(self.config_updated)

    async def get(self, data: LocalData | None = None) -> str | None:
        """Returns full system prompt."""
        await self.refresh_cache()

        timeout = manager.get_config().dynamic_context.function_timeout
    ...
    def get_files(self) -> list[Path]:
        ...
        extensions = config.dynamic_context.file_names
        pattern = "**/*" if config.dynamic_context.recursive else "*"
        files: list[Path] = [

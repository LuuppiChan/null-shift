...
class ModelInfo(BaseModel):
    ...
    # litellm fuckshit
    provider: Literal["openai", "vertexai", "litellm", "openrouter"] | str | None = None
    url: str | None = None
...
class AgentConfig(BaseModel):
    ...
    continue_prompt: str = (
        "[AGENT SYSTEM]: You haven't indicated completion intent. Continue the task or call the agent_complete_objective tool. Remeber to stay in the tool call loop by calling tools every response!"
    )
    ...
    def get_model(self, identifier: ModelIdentifier) -> ModelInfo:
        ...
        if isinstance(identifier, ModelInfo):
            if identifier.name in self.llm.model_tiers:
                return self.llm.default.with_overrides(
                    self.llm.model_tiers[identifier.name]
                )
...
class RuntimeState(BaseModel):
    ...
    pending_permission_requests: list[PendingPermissionRequest] = Field(
        default_factory=list
    )
    pending_permission_responses: list[PendingPermissionRequest] = Field(
        default_factory=list
    )
